...
// [新增] PC端 - 提交到购物车
// ==========================================================
async function submitAddToCartPc() {
    // 1. 验证 (基于PC页面状态)
    if (!selectedVariant) {
        alert('请选择规格');
        try {
            document.getElementById('variant-list-pc').style.border = '1px solid red';
            setTimeout(() => { document.getElementById('variant-list-pc').style.border = 'none'; }, 2000);
        } catch(e){}
        return;
    }
    
    // 2. 验证购买方式 (使用全局变量 buyMode)
    const modeContainerPc = document.getElementById('buy-mode-container-pc');
    if (modeContainerPc && !modeContainerPc.classList.contains('d-none')) { // 检查是否启用了购买方式
        if (!buyMode) {
            alert('请选择购买方式 (随机或自选)');
        ...
        if (buyMode === 'select' && !selectedCardId) {
            alert('请选择号码');
            handlePcBuyModeClick('select'); // 自动打开选择器
            return;
        }
    }
    ...
    // 3. 验证数量 (读取PC端输入框)
    const quantity = parseInt(document.getElementById('buy-qty-pc').value);
    if (buyMode !== 'select') { // 自选模式固定为1，无需检查库存
        if (selectedVariant.stock < quantity) {
            alert('库存不足');
            document.getElementById('buy-qty-pc').focus();
            return;
    ...
    // 4. 执行添加
    const btn = document.getElementById('btn-add-to-cart-pc');
    const oldText = btn.innerText;
    ...
    try {
        ...
        // 使用全局 selectedCardNote (由 selectCard 函数设置)
        const noteToStore = (buyMode === 'select') ? selectedCardNote : null;
        
        // [重要] 价格从 #sku-price-text 读取，
        // 因为 updatePrice() 会始终保持它与PC端计算同步
        const unitPrice = parseFloat(document.getElementById('sku-price-text').innerText);
        ...
        const cartItem = {
            ...
            price: unitPrice, // 存储单价
            quantity: quantity,
            img: document.getElementById('p-img-pc').src, // 使用PC端图片
            buyMode: buyMode,
            selectedCardId: (buyMode === 'select') ? selectedCardId : null,
            selectedCardNote: noteToStore
        };

        // 检查购物车中是否已有“非自选”的同规格商品
        const existingItemIndex = cart.findIndex(item => 
            item.variantId === cartItem.variantId && 
            item.buyMode !== 'select' && // 购物车中的不是自选
            cartItem.buyMode !== 'select' && // 当前添加的也不是自选
            item.buyMode === cartItem.buyMode // 确保模式相同 (例如都是 random)
        );
        ...
        console.error('Add to cart failed (PC)', e);
        alert('添加失败，请重试');
        btn.disabled = false;
        btn.innerText = oldText; 
    }
